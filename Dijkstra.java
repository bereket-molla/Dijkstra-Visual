...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // TODO
        double horizontal = vx - ux; 
        double vertical = vy - uy; 
        double EcDistance = Math.sqrt((horizontal*horizontal) + (vertical*vertical));
        return EcDistance;
  }
  ...
  public void computeAllEuclideanDistances() {
        for ( Vertex source : vertexNames.values() ) {
          for(Edge targetEdge: source.adjacentEdges){
            double ux = source.x; 
            double uy = source.y; 

            double vx = targetEdge.target.x;
            double vy = targetEdge.target.y;
            targetEdge.distance = computeEuclideanDistance(ux, uy, vx, vy);
  ...
  public void doDijkstra(String s) {
        // TODO 
        Vertex initialVertex = vertexNames.get(s);
        // System.out.println(initialVertex); 
        List<Vertex> toBeVisited = new ArrayList<>();
        ...
        while(!toBeVisited.isEmpty()) {
          ...
          // for(Vertex v: toBeVisited){ 
          //   System.out.println(v);
          // }
       
          Vertex smallestNode =  toBeVisited.get(0); 
          for(Vertex v: toBeVisited){
            if( v.distance < smallestNode.distance){
              smallestNode = v;
            }
          }
      
          toBeVisited.remove(smallestNode);
          smallestNode.known = true;

          for( Edge e: smallestNode.adjacentEdges){
            if(!e.target.known){
              double newCost  = smallestNode.distance + e.distance;
              if(newCost < e.target.distance){
                e.target.distance = newCost;
                e.target.prev = smallestNode;
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    while(temp != null ){
      for(Edge e: temp.adjacentEdges){ 
        if((e.target).equals(temp.prev)){
          answer.addFirst(e);
        }
      }
      temp = temp.prev;
